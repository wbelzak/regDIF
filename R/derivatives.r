...
d <-
  function(parm,p,r1,r0,data,item.focus,theta,covariates,cov,num_items,samp_size,num_quadpts) {
    ...
    p_active <- c(p[paste0("c0_itm",item.focus,"_")],
                  p[grep(paste0("c1_itm",item.focus,"_"),names(p),fixed=T)],
                  p[paste0("a0_itm",item.focus,"_")],
                  p[grep(paste0("a1_itm",item.focus,"_"),names(p),fixed=T)])
