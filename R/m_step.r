...
Mstep_2pl_dif <-
  ...
  for(cov in 0:(ncol(mean_predictors)-1)){
    anl_deriv <- d_alpha_est(p[[num_items+1]],p[[num_items+2]],etable_all,elist$theta,mean_predictors,var_predictors,cov=cov,samp_size,num_items,num_quadpts)
  ...
  for(cov in 0:(ncol(var_predictors)-1)){
    anl_deriv <- d_phi_est(p[[num_items+1]],p[[num_items+2]],etable_all,elist$theta,mean_predictors,var_predictors,cov=cov,samp_size,num_items,num_quadpts)
  ...
  for (item in 1:num_items) {
    ...
    if(item.type[item] == "bernoulli"){
      ...
      #intercept updates
      anl_deriv <- d_bernoulli_est("c0",p_item,etable[[1]],etable[[2]],elist$theta,predictor.data,cov=0,samp_size,num_items,num_quadpts)
      ...
      #slope updates
      if(rasch == FALSE){
        anl_deriv <- d_bernoulli_est("a0",p_item,etable[[1]],etable[[2]],elist$theta,predictor.data,cov=0,samp_size,num_items,num_quadpts)
      ...
      if(!any(item == anchor)){
        ...
        for(cov in 0:(num_predictors-1)){
          ...
          anl_deriv <- d_bernoulli_est("c1",p_item,etable[[1]],etable[[2]],elist$theta,predictor.data,cov,samp_size,num_items,num_quadpts)
          z <- p_item[grep(paste0("c1_itm",item,"_cov",cov+1),names(p_item),fixed=T)] - anl_deriv[[1]]/anl_deriv[[2]]
          p_new <- ifelse(penalty == "lasso",soft_thresh_est(z,alpha,tau),firm_thresh_est(z,alpha,tau,gamma))
          names(p_new) <- names(z)
          p_item <- replace(p_item,names(p_new),p_new)
        }
        ...
        for(cov in 0:(num_predictors-1)){
          ...
          if(rasch == FALSE){
            anl_deriv <- d_bernoulli_est("a1",p_item,etable[[1]],etable[[2]],elist$theta,predictor.data,cov,samp_size,num_items,num_quadpts)
      ...
      #intercept updates
      anl_deriv <- d_categorical("c0",p_item,etable,elist$theta,predictor.data,thr=NULL,cov=NULL,samp_size,num_responses[[item]],num_items,num_quadpts)
      p_new <- p_item[grep(paste0("c0_itm",item,"_"),names(p_item),fixed=T)][1] - anl_deriv[[1]]/anl_deriv[[2]]
      p_item <- replace(p_item,names(p_new),p_new)
      ...
      if(num_responses[item] > 2){
        for(thr in 2:(num_responses[item]-1)){
          anl_deriv <- d_categorical("c0",p_item,etable,elist$theta,predictor.data,thr=thr,cov=NULL,samp_size,num_responses[[item]],num_items,num_quadpts)
      ...
      if(rasch == FALSE){ #skip slope estimate updates if rasch is TRUE
        anl_deriv <- d_categorical("a0",p_item,etable,elist$theta,predictor.data,thr=NULL,cov=NULL,samp_size,num_responses[[item]],num_items,num_quadpts)
        p_new <- p_item[grep(paste0("a0_itm",item,"_"),names(p_item),fixed=T)] - anl_deriv[[1]]/anl_deriv[[2]]
        p_item <- replace(p_item,names(p_new),p_new)
      }
      ...
      if(!any(item == anchor)){
        ...
        for(cov in 1:num_predictors){
          ...
          anl_deriv <- d_categorical("c1",p_item,etable,elist$theta,predictor.data,thr=NULL,cov,samp_size,num_responses[[item]],num_items,num_quadpts)
        ...
        for(cov in 1:num_predictors){
          ...
          if(rasch == FALSE){
            anl_deriv <- d_categorical("a1",p_item,etable,elist$theta,predictor.data,thr=NULL,cov,samp_size,num_responses[[item]],num_items,num_quadpts)
      ...
      #intercept updates
      anl_deriv <- d_mu_gaussian("c0",p_item,etable,elist$theta,item.data[,item],predictor.data,cov=NULL,samp_size,num_items,num_quadpts)
      p_new <- p_item[grep(paste0("c0_itm",item,"_"),names(p_item),fixed=T)][1] - anl_deriv[[1]]/anl_deriv[[2]]
      p_item <- replace(p_item,names(p_new),p_new)

      #slope updates
      if(rasch == FALSE){ #skip slope estimate updates if rasch is TRUE
        anl_deriv <- d_mu_gaussian("a0",p_item,etable,elist$theta,item.data[,item],predictor.data,cov=NULL,samp_size,num_items,num_quadpts)
        p_new <- p_item[grep(paste0("a0_itm",item,"_"),names(p_item),fixed=T)] - anl_deriv[[1]]/anl_deriv[[2]]
        p_item <- replace(p_item,names(p_new),p_new)
      }

      #residual updates
      anl_deriv <- d_sigma_gaussian("s0",p_item,etable,elist$theta,item.data[,item],predictor.data,cov=NULL,samp_size,num_items,num_quadpts)
      ...
      if(!any(item == anchor)){
        ...
        for(cov in 1:num_predictors){
          anl_deriv <- d_sigma_gaussian("s1",p_item,etable,elist$theta,item.data[,item],predictor.data,cov=cov,samp_size,num_items,num_quadpts)
        ...
        for(cov in 1:num_predictors){
          ...
          anl_deriv <- d_mu_gaussian("c1",p_item,etable,elist$theta,item.data[,item],predictor.data,cov,samp_size,num_items,num_quadpts)
          z <- p_item[grep(paste0("c1_itm",item,"_cov",cov),names(p_item),fixed=T)] - anl_deriv[[1]]/anl_deriv[[2]]
          p_new <- ifelse(penalty == "lasso",soft_thresh_est(z,alpha,tau),firm_thresh_est(z,alpha,tau,gamma))
          names(p_new) <- names(z)
          p_item <- replace(p_item,names(p_new),p_new)
        ...
        for(cov in 1:num_predictors){
          ...
          if(rasch == FALSE){
            anl_deriv <- d_mu_gaussian("a1",p_item,etable,elist$theta,item.data[,item],predictor.data,cov,samp_size,num_items,num_quadpts)
            z <- p_item[grep(paste0("a1_itm",item,"_cov",cov),names(p_item),fixed=T)] - anl_deriv[[1]]/anl_deriv[[2]]
            p_new <- ifelse(penalty == "lasso",soft_thresh_est(z,alpha,tau),firm_thresh_est(z,alpha,tau,gamma))
            names(p_new) <- names(z)
            p_item <- replace(p_item,names(p_new),p_new)
